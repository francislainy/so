...
const {retrieveStatsForReport} = require("../../api");
const {provider, url, port} = require("../helper");

const {uuid, string, integer, regex, decimal, eachLike} = require('@pact-foundation/pact/dsl/matchers');
...
describe("Stats API test", () => {
    ...
    describe("get /stats/report/86255ceb-b925-451f-9f4d-f8a6e64681a1", () => {
        before(done => {
            const interaction = {
                state: "a request for stats that belong to a report",
                uponReceiving: "a request for stats that belong to a report",
                withRequest: {
                    method: "GET",
                    path: "/api/gatling-tool/stats/report/86255ceb-b925-451f-9f4d-f8a6e64681a1",
                    headers: {
        ...
        it("returns the correct response", done => {
            const axiosParams = {
                url: url,
                port: port,
                id: "86255ceb-b925-451f-9f4d-f8a6e64681a1",
            }

            retrieveStatsForReport(axiosParams).then(response => {
                try {
